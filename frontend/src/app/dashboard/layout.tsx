    ...
    return (
        <div className="flex min-h-screen bg-transparent text-slate-200">
            ...
            <div className="group/sidebar flex flex-col items-center relative transition-all duration-[450ms] ease-in-out w-[80px] hover:w-[260px] h-screen sticky top-0 py-4 px-3 z-50">
                <article className="border border-solid border-slate-700/40 w-full h-full ease-in-out duration-500 rounded-2xl flex flex-col shadow-lg shadow-black/15 bg-slate-900/95 overflow-hidden">
                    
                    <div className="flex items-center w-full h-20 shrink-0 px-2 group-hover/sidebar:px-4">
                        <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-indigo-500 to-pink-500 flex items-center justify-center shrink-0 mx-auto group-hover/sidebar:mx-0 shadow-lg shadow-indigo-500/20">
                            <span className="text-white font-bold text-base">CH</span>
                        </div>
                        <h2 className="ml-4 text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-indigo-400 to-pink-500 whitespace-nowrap overflow-hidden transition-all duration-300 max-w-0 opacity-0 group-hover/sidebar:max-w-xs group-hover/sidebar:opacity-100">
                            Campus Hub
                        </h2>
                    </div>
                            ...
                            return (
                                <Link 
                                    ...
                                    className={`relative w-full h-12 flex flex-row items-center justify-center group-hover/sidebar:justify-start group-hover/sidebar:px-4 rounded-xl transition-all duration-300 ease-in-out border border-transparent group/link
                                      ${isActive 
                                        ? "shadow-lg bg-indigo-500/10 border-indigo-500/30 text-indigo-400" 
                                        : "hover:bg-slate-800/50 text-slate-400 hover:text-indigo-300 hover:border-slate-700/50"}
                                    `}
                                >
                                    <div className={`shrink-0 flex items-center justify-center transition-transform duration-300 ease-in-out
                                        ${isActive ? "scale-110 text-indigo-400" : "group-hover/link:scale-110"}
                                    `}>
                                    ...
                                    <div className="flex items-center justify-between overflow-hidden whitespace-nowrap transition-all duration-300 max-w-0 opacity-0 group-hover/sidebar:max-w-[200px] group-hover/sidebar:opacity-100 group-hover/sidebar:ml-4 flex-1">
                                        ...
                                        {link.name === "Notifications" && unreadCount > 0 && (
                                            <span className="ml-2 bg-pink-500 text-white text-[10px] flex items-center justify-center font-bold px-2 py-0.5 rounded-full shadow-[0_0_10px_rgba(236,72,153,0.5)]">
                                                {unreadCount}
                                            </span>
                                        )}
                                    </div>
                                    ...
                                    {link.name === "Notifications" && unreadCount > 0 && (
                                        <span className="absolute top-2 right-2 flex w-2 h-2 bg-pink-500 rounded-full shadow-[0_0_10px_rgba(236,72,153,0.8)] opacity-100 group-hover/sidebar:opacity-0 transition-opacity"></span>
                                    )}
                                </Link>
                            );
                        })}
                    ...
                    <div className="w-full p-2 border-t border-slate-700/40">
                        <button 
                            onClick={async () => {
                                try {
                                    const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080";
                            ...
                            className="relative w-full h-12 flex flex-row items-center justify-center group-hover/sidebar:justify-start group-hover/sidebar:px-4 rounded-xl transition-all duration-300 ease-in-out border border-transparent text-red-500 hover:bg-red-500/10 hover:border-red-500/30 group/link"
                        >
